...
class ProdLine:
    ...
    def has_tool(this, tool, both = True):
        return (tool in this.m1t and both) or tool in this.m2t
    
    def has_tools(this, tools):
        first = True
        tot = 0
        for i in tools:
            if i in this.m1t and first:
                tot+=1
            else:
                first = False
            if not first:
...
class EndLine:
    ...
    def send(this, piece):
        if(this.free_cells > 0):
            ...
            entry_list[11] = piece
            this.entry_node.set_value(ua.Variant(entry_list, ua.VariantType.Int16))
            print(f"Extracted final piece {piece} from WH2 to CX_entry_piece")
            return True
        return False
        ...
        with open(os.path.join(orders_folder, filename), 'r') as file:
            ...
            for order in data.get("orders", []):
                ...
                for i in range(quantity):
                    order_queue.append(order_type)
                    print("ADDED ",order_type)
...
def read_codesys_variables():
    ...
    try:
        ...
        trans_m_node_3 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C3_transformations_M")
        trans_t_node_3 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C3_transformations_T")
        trans_m_node_4 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C4_transformations_M")
        trans_t_node_4 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C4_transformations_T")
        trans_m_node_5 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C5_transformations_M")
        trans_t_node_5 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C5_transformations_T")
        trans_m_node_6 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C6_transformations_M")
        trans_t_node_6 = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.C6_transformations_T")
        ...
        end_of_day_node = client.get_node("ns=4;s=|var|CODESYS Control Win V3 x64.Application.GVL.End_of_day")

        end_of_day_node.set_value(ua.Variant(0, ua.VariantType.Boolean))
        ...
        prodLines=[
            ...
            ProdLine(entry_node, trans_m_node_3, trans_t_node_3, cell_free_node, cell_steps_node, [3,4,5], [4,5,6], 6),
            ProdLine(entry_node, trans_m_node_4, trans_t_node_4, cell_free_node, cell_steps_node, [4,5,6], [5,6,1], 7),
            ProdLine(entry_node, trans_m_node_5, trans_t_node_5, cell_free_node, cell_steps_node, [5,6,1], [6,1,2], 8),
            ProdLine(entry_node, trans_m_node_6, trans_t_node_6, cell_free_node, cell_steps_node, [6,1,2], [1,2,3], 9)
        ]

        endLines=[
            EndLine(entry_node, cell_free_node, 11),
            EndLine(entry_node, cell_free_node, 12),
            EndLine(entry_node, cell_free_node, 13),
            EndLine(entry_node, cell_free_node, 14)
        ]

        while len(order_queue) > 0:
            print(len(order_queue),"LEFT")
            order_type = order_queue.popleft()
            ...
            if result_piece is None:
                ...
                try:
                    ...
                    if result_piece == order_type:
                        i = 0
                        while not endLines[i].send(result_piece):
                            i+=1
                        if(i >= 4):
                            print("Warning: End lines are full!")
                        else:
                            WH2[index] = 0
        ...
        end_of_day_node.set_value(ua.Variant(1, ua.VariantType.Boolean))
        time.sleep(10)
        end_of_day_node.set_value(ua.Variant(0, ua.VariantType.Boolean))
